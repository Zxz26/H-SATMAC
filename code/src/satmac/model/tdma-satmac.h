...
void NotifyConflictDetected(Time conflictTime, Ptr<Node> conflictNode);

protected:

private:
  bool slotgroup_ena;//是否启用时隙组
	int m_slot_group;//本节点时隙组
	int slot_group_length;//时隙组长度
	long long total_slot_group_count;
	int slot_group_count;//当前时隙组index
	bool adj_ena_sg;//时隙调整为了时隙组
	bool isNeedSg;//是否需要时隙组
	bool isNeed_adj_Sg;//是否需要调整时隙为了时隙组
  double adjRatio_low_sg;
  double adjRatio_high_sg;

	std::array<slot_group_info, SLOT_GROUP_LENGTH> m_sg_info;
	...
	void ExecuteSlotGroupOperations(int maxFreeSlots, int startSlot, int endSlot);

	int GetGeohash();
	int determine_SG();
	void merge_sgi(SlotGroupHeader recv_sgi_hdr);
	void CheckIsNeedSG();
	double get_available_slot_group_ratio();
	int adj_BCH_Sg();
	void sg_opration(Ptr<Packet> p);
  void SendSgiDown(Ptr<Packet> packet, WifiMacHeader header);

	std::string m_channel_utilization_OutFile;
