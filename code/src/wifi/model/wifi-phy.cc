...
WifiPhy::NotifyTxBegin (Ptr<const Packet> packet)
{
  ...
  BsmTimeTag apptag;
  AperiodicTag aptag;
  if(packet->PeekPacketTag(apptag))
  {
    if(packet->PeekPacketTag(aptag))
    {
        TxCounter::GetInstance().Increment_aperiodic();
    }
    else
    {
        TxCounter::GetInstance().Increment_periodic();
    }
  }
}
...
WifiPhy::SendPacket (Ptr<const Packet> packet, WifiTxVector txVector, MpduType mpdutype)
{
  ...
  Ptr<Node> node = this->GetDevice()->GetNode();
  Ptr<WifiNetDevice> device = DynamicCast<WifiNetDevice>(this->GetDevice());
  ...
  if(device == tdma)
  {
	  //std::cout<<"tdma "<<node->GetId()<<" Time: "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  }
  else
  {
	  //std::cout<<"csma "<<node->GetId()<<" Time: "<<Simulator::Now().GetMicroSeconds()<<std::endl;
  }
